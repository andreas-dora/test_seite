function init(){
    ...
    var bewegung = routeForm.bewegung.value;
    // var bewegung = 'driving-traffic';
    ...
    // const end = "Frankfurt am Main, Berger Str. 112"; // Gutleustr. 32
  ...
  async function getRoute() {
    ...
    // );
    const query = await fetch(
      `https://api.mapbox.com/directions/v5/mapbox/${bewegung}/${start[0]},${start[1]};${end[0]},${end[1]}?steps=true&language=de&geometries=geojson&access_token=${mapboxgl.accessToken}`,
      { method: 'GET' }
    );
    ...
    let km = (data.distance /1000).toString().padStart(0, 2);
    
    dauer.value = `ca. € ${data.distance} | Entfernung: ${km}km | Dauer: ${Math.floor(
    ...
    instructions.innerHTML = `<p><strong>Bewegung: ${bewegung}</strong></p><br
